...
function menuSubsToggleContent(id,para,val) {
    var elem = document.getElementById(id);
    if(!elem)
        return true;

    var s = new RegExp(para+"(\ )*:(\ )*"+val+"(\ )*(;)?");
    var b = new RegExp(para+"(\ )*\:(\ )*([^;]+)([a-z-])+(;)?");
    var string = elem.getAttribute("style");
    var safe_style_value = elem.getAttribute("data-menusubs");
    if(!string)
        string = "";
